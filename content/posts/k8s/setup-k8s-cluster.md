...
Chạy lệnh sau trên Master Node. Lưu ý `--pod-network-cidr` phải khớp với plugin mạng bạn định dùng (ở đây ta dùng **Calico** nên để dải IP này).
...
Cluster đã chạy nhưng các Pod chưa thể nói chuyện với nhau. Ta cần cài CNI. Ở đây dùng **Calico**.
...
> *   Calico là một plugin thực thi chuẩn đó, giúp tạo ra mạng ảo (Overlay Network) để Pod ở Node A có thể ping thấy Pod ở Node B.
...
*   Nếu trạng thái là `NotReady`: Chờ khoảng 1-2 phút để CNI (Calico) khởi động xong.
